...
# - Исправлена проблема с async event loop в SemanticSearchEngine
# - Добавлен синхронный метод _fit_sync для избежания конфликтов в Jupyter
# - Отключено параллельное обучение в HybridSearchEngine по умолчанию
# - Добавлен fallback к последовательному обучению при ошибках параллельного
...
try:
    ...
    dataset_path = data_helper["datasets"] / "Выгрузка_Номенклатура_без_удаленных_17_07_25.xlsx"
...
print(f"Дубликаты: {data.duplicated(subset=[main_name_column], keep=False).sum()}")
...
# 3. Гибридный поиск
# ИСПРАВЛЕНИЕ: Отключено параллельное обучение для избежания async конфликтов в Jupyter
if fuzzy_engine and semantic_engine:
    try:
        hybrid_config = create_safe_config(
            ...
            enable_parallel_search=False,  # ИСПРАВЛЕНО: Отключено для избежания async конфликтов
        )
